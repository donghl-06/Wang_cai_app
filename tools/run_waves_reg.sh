#!/bin/bash
# 分波驱动(注册制切换 2023-03~04,流水版):股票块(150只) × 周日历块(≈5 个交易日) 双层分波
#   校验第 N 波的同时后台预取第 N+1 波(独立暂存目录),拉取时间被校验时间覆盖
#   每子波:入库 → 校验 → 全过则删原始数据 → 提交结果 → 下一子波
# 用法: nohup bash tools/run_waves_reg.sh > logs/waves_reg.log 2>&1 &
# 进度: tail -f logs/waves_reg.log(明细 logs/fetch_reg.log、logs/validate_reg.log)
...
END=2023-04-30
UNIVERSE=600
UNIVERSE_FILE=adata_universe_reg_600.txt
BLOCK_SIZE=150         # 股票块大小(3 块 × 150 = 450)
CHUNK_DAYS=7           # 日历天/子波(≈5 个交易日)
RESULTS=adata_validation_results_reg.csv
STATE=logs/waves_reg.state   # 最后完成的子波 "<offset> <chunk_start>"
STAGING=adata_staging_reg    # 预取暂存根目录(每子波一个子目录,校验目录不受污染)
...
fetch_wave() {
    ...
    $FETCH_PY fetch_adata_logs.py --start "$cs" --end "$ce" \
        ...
        --universe-file "$UNIVERSE_FILE" --out "$out" >> logs/fetch_reg.log 2>&1
}
...
if (( start_idx < TOTAL )); then
    ...
    until fetch_wave $start_idx adata_logs; do
        log "❌ 拉取失败,见 logs/fetch_reg.log,60s 后重试"
        sleep 60
    done
fi
...
for (( i=start_idx; i<TOTAL; i++ )); do
    ...
    until $VENV_PY tests/price_cage/run_adata_validation.py --batch-size 12 \
        --results "$RESULTS" >> logs/validate_reg.log 2>&1; do
        log "❌ 校验进程异常退出,见 logs/validate_reg.log;保留数据,60s 后重试"
        sleep 60
    done
...
EOF
    ...
    if git add "$RESULTS" "$UNIVERSE_FILE" 2>/dev/null && \
       git commit -q -m "adata 注册制切换分波校验: $tag $n_pass/$n_done pass"; then
        log "进度已提交"
    else
        log "无新结果可提交"
    fi
